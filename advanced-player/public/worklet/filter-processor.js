...
class FilterProcessor extends AudioWorkletProcessor {
  ...
  presetFilter(type, option){
    ...
    process(inputs, outputs) {
      ...
      console.log ('process unit: ', input[0].length);

      if (output.length !== 2) return;

// modify below for 128 samples i/o
...
//       let inputData = inputBuffer.getChannelData(channel);
        ...
        for (let sample = 0; sample < 2*this.shiftSize; sample++){ 
          if (sample < this.shiftSize) 
            ...
            fftWindowInput[channel][sample] 
                      = inputData[sample - this.shiftSize];
     ...
     let fftObj = this.justFFT(fftWindowInput); // for test
...
// Add two outputs in overlapped hann window 
   ...
   justFFT (fftWindowInput){
    ...
    let fftCoef = [
       this.rfft.forward(Windowing.hann(fftWindowInput[0])).slice(),
       this.rfft.forward(Windowing.hann(fftWindowInput[1])).slice()];
    // 0: Left, 1: Right

      let power = [
        new Float32Array(this.shiftSize+1),
        new Float32Array(this.shiftSize+1)
      ]; // 0: Left, 1: Right
